  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="fixed inset-0 z-0">
        {viewMode === 'default' && (
          <Threads color={[0.5, 0.5, 0.5]} amplitude={0.6} distance={0.2} />
        )}
        {viewMode === 'students' && (
          <Threads color={[0.5, 0.5, 0.5]} amplitude={0.6} distance={0.2} backgroundColor="#000000" />
        )}
        {viewMode === 'doctors' && (
        ...
        {viewMode === 'default' ? (
          <div className="flex flex-col items-center -mt-16">
            <img
              ...
              className="w-[280px] h-[280px] md:w-[400px] md:h-[400px] lg:w-[500px] lg:h-[500px] object-contain cursor-pointer hover:scale-105 transition-transform duration-300"
            />
            <GradientText
      ...
      <div 
        className={`fixed top-4 left-4 md:top-6 md:left-6 z-50 transition-opacity duration-500 will-change-opacity ${isScrolled ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      >
        <img
          src={viewMode === 'students' ? '/medvora_logo_white.png' : '/medvora_logo.png'}
          alt="Medvora"
      ...
      {/* Top fade overlay */}
      <div 
        className={`fixed top-0 left-0 right-0 h-24 z-40 pointer-events-none transition-opacity duration-300 will-change-opacity ${isScrolled ? 'opacity-100' : 'opacity-0'}`}
        style={{
